...
const SALES_LOG_KEY = "pcplus_sales_v1";
...
function enforcePublisherNavOrder() {
  ...
  const profile =
    nav.querySelector('a[href="../customer/profile.html"]') ||
    nav.querySelector('a[href="publisher-profile.html"]');
  const state = nav.querySelector(".nav-login-state");

  if (profile) profile.setAttribute("href", "../customer/profile.html");
...
function bindTopbarSearchRedirect() {
  ...
  const hideSearch = !!(s && s.role === "publisher");
  if (searchWrap) searchWrap.style.display = hideSearch ? "none" : "";
...
// Injects clickable "Logged in <avatar>" auth tab into publisher nav.
function renderPublisherStatus() {
  ...
  const stateLabel = document.createElement("a");
  stateLabel.className = "nav-login-state";
  stateLabel.textContent = "Logged in " + avatar;
  stateLabel.href = "../customer/login.html";
...
function renderDashboard() {
  ...
  // Primary revenue source: exact paid amounts from completed cart checkouts.
  const estimateFromSalesLedger = () => {
    let entries = [];
    try {
      entries = JSON.parse(localStorage.getItem(SALES_LOG_KEY) || "[]");
    } catch {
      entries = [];
    }
    ...
    const publisherKeys = new Set(
      games.map((g) => normalizeStoreKey(g.storeKey)).filter(Boolean)
    );
    ...
    entries.forEach((entry) => {
      ...
      items.forEach((item) => {
        const key = normalizeStoreKey(item?.storeKey || item?.id);
        if (!publisherKeys.has(key)) return;
  ...
  // Fallback for old data created before checkout ledger existed.
  const estimateFromLibraryLegacy = () => {
    let owned = [];
    try {
      owned = JSON.parse(localStorage.getItem("pcplus_library") || "[]");
    } catch {
      owned = [];
    }
  ...
  const ledgerRevenue = estimateFromSalesLedger();
  const estimatedRevenue = ledgerRevenue === null ? estimateFromLibraryLegacy() : ledgerRevenue;
